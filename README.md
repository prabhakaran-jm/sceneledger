...
**M3** adds provenance and release verification: the release manifest links source chunks, scene plan, stale report, and M2 media manifests with live SHA-256 checks, plus `GET /release` and `POST /verify-release`.
...
6. **Create Release Manifest** — Release Evidence card shows `verified`, hash checklist, per-scene assets

7. **Verify Release** — same `release_id`, `hash_verified: true`

8. **Upload Source v2** (only paragraph 3 changed: assembly point A → C)

9. **Compare Source Versions** — scene-003 stale, scenes 1–2 current

10. **Create Release Manifest** again — `warning`, `release_superseded_by_source_version: "v2"`, scene-003 stale

11. **Refresh Stored Objects** / **Refresh Media** — lists keys and per-scene asset metadata
...
curl -s "http://localhost:8000/projects/${PROJECT_ID}/release?source_version=v1"



curl -s -X POST "http://localhost:8000/projects/${PROJECT_ID}/verify-release" \

  -H "Content-Type: application/json" \

  -d "{\"source_version\":\"v1\",\"update_manifest\":false}"
...
Expected release before compare: `release_status` is `verified`, `hash_verified` is `true`.

Expected release after compare: `release_status` is `warning`, `release_superseded_by_source_version` is `"v2"`.
...
1. After **Generate Media**, **Create Release Manifest** → `release_status: verified`, `message` confirms hashes verified

2. **Verify Release** → same `release_id`, `errors: []`

3. After v1/v2 compare, **Create Release Manifest** → `release_status: warning`, `release_superseded_by_source_version: "v2"`
...
1. Complete a verified release

2. Edit one byte in `.sceneledger/projects/{id}/media/v1/scene-001/storyboard.png`

3. **Verify Release** → `release_status: blocked`, `hash_verified: false`



**B2 mode:**

1. Complete a verified release in B2

2. In the Backblaze bucket UI, delete or replace a media object (e.g. `storyboard.png`) under `tenants/demo/projects/{id}/media/v1/...`

3. **Verify Release** → `blocked` (missing asset or hash mismatch)
...
| **M3** (current) | Release provenance, SHA-256 verification, verify-release |

| **M4** | Final stitched video compose |
...
| POST | `/projects/{id}/release` | Build release manifest with provenance |

| GET | `/projects/{id}/release` | Load stored release manifest |

| POST | `/projects/{id}/verify-release` | Re-verify asset hashes (optional manifest update) |
